      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col animate-slide-in-right">
        <div className="flex items-center justify-between p-5 border-b border-app-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-medium">Ваша корзина</h2>
            <span className="px-2 py-0.5 text-xs font-semibold bg-app-cream rounded-full">{items.length} товаров</span>
          </div>
        ...
        <div className='flex-1 overflow-y-auto p-5 space-y-4'>
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <h3 className='text-lg font-medium mb-1'>Ваша корзина пуста</h3>
            </div>
          ) : (
            items.map((item) => <div key={item.product._id} className='flex gap-3 bg-app-cream/60 rounded-xl p-3'>
        ...
        {items.length > 0 && (
            <div className='p-5 border-t border-app-border space-y-3'>
                ...
                <div className='flex justify-between text-sm'>
                    ...
                    <span className='font-medium'>{deliveryFee === 0 ? <span className='text-app-success'>Бесплатно</span> : `${currency}${deliveryFee.toFixed(2)}`}</span>
                </div>
                {deliveryFee > 0 && <p className='text-xs text-app-text-light text-center'>Бесплатная доставка при заказе от {currency}20!</p>}
                ...
                <button onClick={() => {setIsCartOpen(false); navigate('/checkout'); window.scrollTo(0, 0)}} className='w-full py-3 bg-app-orange text-white font-semibold rounded-xl hover:bg-app-orange-dark transition-colors flex-center gap-2 active:scale-[0.98]'>
                    Оформить заказ <ArrowRightIcon className='size-4'/>
                </button>
            </div>
        )}
      </div>
