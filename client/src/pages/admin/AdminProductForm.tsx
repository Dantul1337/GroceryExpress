        ...
        <>
            <div className="bg-white rounded-2xl shadow-sm border border-app-border overflow-hidden">
                <div className="px-6 py-5 border-b border-app-border flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-semibold text-zinc-900">{isEdit ? "Редактировать товар" : "Новый товар"}</h2>
                </div>
                {loading ? (
                    <Loading />
                ) : (
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-2">Цена до скидки (₽) - необязательно</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-2">Единица измерения</label>
                                <input required type="text" placeholder="напр., кг, шт, л" value={formData.unit} onChange={e => setFormData({ ...formData, unit: e.target.value })} className="w-full px-4 py-2.5 rounded-lg border border-zinc-200 focus:border-app-green focus:ring-1 focus:ring-app-green outline-none transition-all" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-2">Остаток на складе</label>
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-zinc-700 mb-2">Изображение товара</label>
                                <div className="flex items-center gap-4">
                                    {(imageFile || formData.image) && (
                                        <div className="size-16 rounded-lg border border-zinc-200 overflow-hidden shrink-0 bg-app-cream">
                                            <img src={imageFile ? URL.createObjectURL(imageFile) : formData.image} alt="Предпросмотр" className="w-full h-full object-cover" />
                                        </div>
                                    )}
                            ...
                            <div className="flex items-center gap-3">
                                <label htmlFor="isOrganic" className="text-sm font-medium text-zinc-700 cursor-pointer">Органический продукт</label>
