...
import { dummyDashboardOrdersData, dummyDeliveryPartnerData, statusLabels } from "../../assets/assets";
        ...
        <>
            <div className="bg-white rounded-2xl shadow-sm border border-app-border overflow-hidden">
                <div className="px-6 py-5 border-b border-app-border">
                    <h2 className="text-xl font-semibold text-zinc-900">Заказы</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        <thead className="bg-app-cream/50 text-zinc-500 uppercase text-xs font-semibold">
                            <tr>
                                <th className="px-6 py-4">Информация о заказе</th>
                        ...
                        <tbody className="divide-y divide-app-border">
                            {orders.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-zinc-500">Заказы не найдены.</td>
                                </tr>
                            ) : (
                                orders.map((order: any) => (
                                    <tr key={order._id} className="hover:bg-zinc-50/50 transition-colors">
                                        <td className="px-6 py-4">
                                            ...
                                            <p className="text-xs text-zinc-500">{new Date(order.createdAt).toLocaleString("ru-RU")}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="font-medium text-zinc-900">{order.user?.name || "Неизвестный пользователь"}</p>
                                            <p className="text-xs text-zinc-500">{order.user?.email || "Нет email"}</p>
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            {order.deliveryPartner ? (
                                                ...
                                                <button onClick={() => { setAssignModal(order._id); setSelectedPartner(""); }} className="px-3 py-1.5 text-xs font-medium bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 transition-colors flex items-center gap-1">
                                                    <TruckIcon className="size-3" /> Назначить
                                                </button>
                                            )}
                                        </td>
                                        <td className="px-6 py-4">
                ...
                <>
                    ...
                    <div className="fixed inset-0 z-50 flex-center p-4">
                        <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-fade-in">
                            <h3 className="text-lg font-semibold text-app-green mb-4">Назначить курьера</h3>
                            {partners.length === 0 ? (
                                <p className="text-sm text-zinc-500 mb-4">Нет активных курьеров. Сначала добавьте курьера.</p>
                            ) : (
                                    ...
                                    {partners.map((p) => (
                                        <label key={p._id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${selectedPartner === p._id ? "border-app-green bg-app-green/5" : "border-app-border hover:bg-app-cream"}`}>
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-zinc-500">{p.vehicleType === "bike" ? "велосипед" : p.vehicleType} • {p.phone}</p>
                                            </div>
                                        </label>
                                    ))}
                                </div>
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setAssignModal(null)} className="flex-1 py-2.5 text-sm font-medium text-zinc-600 bg-zinc-100 rounded-xl hover:bg-zinc-200 transition-colors">Отмена</button>
                                <button onClick={handleAssign} disabled={!selectedPartner} className="flex-1 py-2.5 text-sm font-medium text-white bg-app-green rounded-xl hover:bg-app-green-light transition-colors disabled:opacity-50">Назначить</button>
                            </div>
                        </div>
                    </div>
                </>
