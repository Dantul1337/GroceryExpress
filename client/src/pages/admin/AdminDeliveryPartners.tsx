    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h1 className="text-xl font-semibold text-zinc-900">Курьеры</h1>
                <button onClick={() => setShowForm(true)} className="px-4 py-2 bg-app-green text-white text-sm font-semibold rounded-xl hover:bg-app-green-light transition-colors flex items-center gap-2">
                    <PlusIcon className="size-4" /> Добавить курьера
                </button>
            </div>
            ...
            {partners.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-2xl border border-app-border">
                    ...
                    <p className="text-lg font-semibold text-zinc-900 mb-1">Нет курьеров</p>
                    <p className="text-sm text-zinc-500">Добавьте первого курьера, чтобы начать</p>
                </div>
            ) : (
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-zinc-500">{p.vehicleType === "bike" ? "велосипед" : p.vehicleType === "scooter" ? "самокат" : p.vehicleType === "car" ? "автомобиль" : p.vehicleType}</p>
                                    </div>
                                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex-center p-4">
                        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 w-full max-w-lg animate-fade-in">
                            <div className="flex items-center justify-between mb-5">
                                <h2 className="text-lg font-semibold text-app-green">Добавить курьера</h2>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-app-green mb-1.5">Полное имя</label>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-app-green mb-1.5">Пароль</label>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-sm font-medium text-app-green mb-1.5">Телефон</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-app-green mb-1.5">Тип транспорта</label>
                            ...
                            <button type="submit" disabled={saving} className="mt-6 w-full py-3 bg-app-green text-white font-semibold rounded-xl hover:bg-app-green-light transition-colors disabled:opacity-60">
                                {saving ? "Создание..." : "Добавить курьера"}
                            </button>
                        </form>
                    </div>
                </>
